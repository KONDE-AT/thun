...
let $archeBaseUrl := "https://id.acdh.oeaw.ac.at/thun"
let $versionNo := ""
...
let $auth := request:get-parameter('auth', 'USERNAME:PASSWORD')
...
for $x in xmldb:get-child-collections($config:data-root)
    for $doc in xmldb:get-child-resources($config:data-root||'/'||$x)
        ...
        let $node := try {
                        doc(string-join(($config:data-root, $x, $doc), '/'))//tei:publicationStmt[not(.//tei:idno[@subtype])]/tei:p[1]
                    } catch * {
                        false()
                    }
        return 
                ...
                then
                    ...
                    let $handle := if ($response/@statusCode="201") 
                        then
                            ...
                            $url
                    let $idno := <idno type="URI" subtype="handle">{$handle}</idno>
